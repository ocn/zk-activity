...
pub async fn process_killmail(app_state: &Arc<AppState>, zk_data: &ZkData) -> Vec<Subscription> {
    ...
    for (guild_id, subscriptions_vec) in subscriptions_map.iter() {
        info!(
            ...
            kill_id, subscriptions_vec.len(), guild_id.0
        );
...
async fn evaluate_filter(filter: &Filter, zk_data: &ZkData, app_state: &Arc<AppState>) -> bool {
    ...
    match filter {
        Filter::TotalValue { min, max } => {
            ...
            min.map_or(true, |m| total_value >= m as f64)
                && max.map_or(true, |m| total_value <= m as f64)
        }
        Filter::DroppedValue { min, max } => {
            let dropped_value = zk_data.zkb.dropped_value;
            min.map_or(true, |m| dropped_value >= m as f64)
                && max.map_or(true, |m| dropped_value <= m as f64)
        }
        Filter::Region(region_ids) => get_system(app_state, killmail.solar_system_id)
            .await
            .map_or(false, |s| region_ids.contains(&s.region_id)),
        ...
        Filter::Alliance(alliance_ids) => {
            killmail
                .victim
                .alliance_id
                .map_or(false, |id| alliance_ids.contains(&id))
                || killmail
                    .attackers
                    .iter()
        ...
        Filter::Corporation(corporation_ids) => {
            killmail
                .victim
                .corporation_id
                .map_or(false, |id| corporation_ids.contains(&id))
        ...
        Filter::Character(character_ids) => {
            killmail
                .victim
                .character_id
                .map_or(false, |id| character_ids.contains(&id))
                || killmail.attackers.iter().any(|a| {
                    a.character_id
                        .map_or(false, |id| character_ids.contains(&id))
                })
        ...
        Filter::ShipType(ship_type_ids) => {
            ship_type_ids.contains(&killmail.victim.ship_type_id)
                || killmail.attackers.iter().any(|a| {
                    a.ship_type_id
                        .map_or(false, |id| ship_type_ids.contains(&id))
                })
        }
