...
use crate::config::{AppState, Subscription, Action, FilterNode, Filter, save_subscriptions_for_guild};
use serenity::model::prelude::command::CommandOptionType;
use tracing::error;

pub struct SubscribeCommand;

fn parse_ids<T: std::str::FromStr>(options: &[serenity::model::application::interaction::application_command::CommandDataOption], name: &str) -> Option<Vec<T>> {
    if let Some(CommandDataOptionValue::String(s)) = get_option_value(options, name) {
        let ids: Vec<T> = s.split(',').filter_map(|id| id.trim().parse::<T>().ok()).collect();
...
impl Command for SubscribeCommand {
    ...
    fn register<'a>(&self, command: &'a mut CreateApplicationCommand) -> &'a mut CreateApplicationCommand {
        command
            ...
            .create_option(|option| option.name("id").description("A unique identifier for this subscription.").kind(CommandOptionType::String).required(true))
            .create_option(|option| option.name("description").description("A description for the subscription.").kind(CommandOptionType::String).required(true))
            .create_option(|option| option.name("min_value").description("Minimum total value of the killmail in ISK.").kind(CommandOptionType::Integer))
            .create_option(|option| option.name("max_value").description("Maximum total value of the killmail in ISK.").kind(CommandOptionType::Integer))
            .create_option(|option| option.name("region_ids").description("A comma-separated list of region IDs to include.").kind(CommandOptionType::String))
            .create_option(|option| option.name("system_ids").description("A comma-separated list of system IDs to include.").kind(CommandOptionType::String))
            .create_option(|option| option.name("alliance_ids").description("A comma-separated list of alliance IDs to match.").kind(CommandOptionType::String))
            .create_option(|option| option.name("corp_ids").description("A comma-separated list of corporation IDs to match.").kind(CommandOptionType::String))
            .create_option(|option| option.name("char_ids").description("A comma-separated list of character IDs to match.").kind(CommandOptionType::String))
    ...
    async fn execute(&self, ctx: &Context, command: &ApplicationCommandInteraction, app_state: &Arc<AppState>) {
        ...
        let min_value = get_option_value(options, "min_value").and_then(|v| if let CommandDataOptionValue::Integer(i) = v { Some(*i as u64) } else { None });
        let max_value = get_option_value(options, "max_value").and_then(|v| if let CommandDataOptionValue::Integer(i) = v { Some(*i as u64) } else { None });
        if min_value.is_some() || max_value.is_some() {
            filters.push(Filter::TotalValue { min: min_value, max: max_value });
        }

        if let Some(ids) = parse_ids::<u32>(options, "region_ids") { filters.push(Filter::Region(ids)); }
        if let Some(ids) = parse_ids::<u32>(options, "system_ids") { filters.push(Filter::System(ids)); }
        if let Some(ids) = parse_ids::<u64>(options, "alliance_ids") { filters.push(Filter::Alliance(ids)); }
        if let Some(ids) = parse_ids::<u64>(options, "corp_ids") { filters.push(Filter::Corporation(ids)); }
        if let Some(ids) = parse_ids::<u64>(options, "char_ids") { filters.push(Filter::Character(ids)); }
        if let Some(ids) = parse_ids::<u32>(options, "ship_type_ids") { filters.push(Filter::ShipType(ids)); }
        if let Some(ids) = parse_ids::<u32>(options, "ship_group_ids") { filters.push(Filter::ShipGroup(ids)); }
        ...
        let root_filter = if filters.len() > 1 {
            ...
            filters.pop().map(FilterNode::Condition).unwrap_or(FilterNode::And(vec![])) // Match all if no filters
        };
